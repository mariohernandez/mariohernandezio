---
date: "2024-06-02"
title: "Using ViteJS for your Drupal front-end build"
...
---
I consider myself pretty loyal to the tools I use on a daily basis as a web developer. I am not the kind of dev who jumps on the new and shiny stuff just because it looks cool or others use it. I learned early on in my career that if the tools you use do exactly what you need and do it well, there is no need to switch to the latest tools as they come out. Don't get me wrong, I try things and experiment with them but if at the end of the day I am not getting any more value than what I am getting out of my current tools, I keep using the tools I am familiar with.

Recently at my workplace we decided to move out of the current design system we were using, Patternlab, and into a better and more modern one, Storybook. We also took this opportunity to look at what other tools could be replaced to improve our current front-end workflow. We had been using Gulp as the build tool and at a personal level I have been using Gulp for almost 10 years. We knew swithing design systems would require us to change to a different build tool. We thought the obvious choice was Webpack, but as part of our research I discovered [ViteJS](https://vitejs.dev).

Vite is considered the _Next Generation Frontend Tooling_, and during our testing we were extremely impressed not only with how fast it is, but also with its plugins ecosystem as well as its community support. Compared to other build tools Vite is relatively new, but it has been around for a few years now and it has matured to the point I feel confident using it. I'll let you read about Vite's features and how it compares to other build tools. Next I am going to focus on building an automated workflow for your Drupal front-end or theme. Mind you, most of the things we'll do apply to any project type, not just Drupal.

## Tasks to automate

As a front-end developer, there are tasks that need to run constantly while working on a project. Running these tasks by hand everytime they are needed would be extremely repeatitive and unefficient among other things. Build tools like Vite make it possible to automate these tasks and they run in the background without getting in the way of our work. Here are some of the tasks we will setup to run automatically:
